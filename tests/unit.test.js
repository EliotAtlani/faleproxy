...
describe('Yale to Fale replacement logic', () => {
  ...
  test('should handle text that has no Yale references', () => {
    const htmlWithoutYale = `
      ...
      <body>
        <h1>Hello World</h1>
        <p>This is a test page with no such references.</p>
      </body>
      </html>
    `;
    ...
    expect(modifiedHtml).toContain('<p>This is a test page with no such references.</p>');
  });
  ...
  test('should handle case-insensitive replacements', () => {
    ...
    $('body *').contents().filter(function() {
      ...
      const text = $(this).text();
      // Replace while preserving case: Yale -> Fale, yale -> fale, YALE -> FALE
